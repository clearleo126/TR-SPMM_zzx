...
__global__ void tr_sptc_kernel(
    const int* __restrict__ sptc_chunk_win,     // [num_chunks] 每 chunk 的窗口 id
...
#pragma unroll
    ...
    // =========================================================================
    // 写回 - MP 布局: 每线程 8 列 (列 = warp_feat_base + tid*8 + {0..7}),
    //   每列 2 行 (row0, row1). 用 float2 atomicAdd (4 次/行 × 2 行)
    //
